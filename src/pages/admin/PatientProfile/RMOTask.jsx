...
import { CheckCircle, Heart, Plus, X, Trash2, Edit } from 'lucide-react';
import { useOutletContext } from 'react-router-dom';

const PREDEFINED_RMO_TASKS = [
  'Information Received',
  'Update to Consultant',
  'Update Opinion',
  'Counseling Patient Family',
  'Financial Counselling',
  'OT Information (Surgical or Non Surgical)'
];
...
export default function RMOTask() {
  ...
  useEffect(() => {
    ...
    let pending = [];
    try {
      const stored = localStorage.getItem('rmoTasksPending');
      if (stored) {
    ...
    if ((!pending || pending.length === 0) && hist.length === 0) {
      const initialTasks = PREDEFINED_RMO_TASKS.map((task, index) => ({
        taskId: `RMO-${Date.now()}-${index}`,
        taskNo: `RMO-${String(index + 1).padStart(3, '0')}`,
        taskName: task,
        assignBy: 'Admin',
        rmoName: 'Dr. Smith',
        nurseName: 'Nurse Joy',
      ...
      localStorage.setItem('rmoTasksPending', JSON.stringify(initialTasks));
    } else {
  ...
  const handleSubmitSelected = () => {
    ...
    try {
      localStorage.setItem('rmoTasksPending', JSON.stringify(remainingPending));
      localStorage.setItem('rmoTasksHistory', JSON.stringify(updatedHistory));
    } catch (e) {
  ...
  const handleSubmit = () => {
    ...
    const getNextTaskNo = () => {
      ...
      let maxNum = 0;
      allTasks.forEach(t => {
        const num = parseInt((t.taskNo || 'RMO-000').replace(/^RMO-/, ''), 10);
        if (num > maxNum) maxNum = num;
      });
      return `RMO-${String(maxNum + 1).padStart(3, '0')}`;
    };
    ...
    localStorage.setItem('rmoTasksPending', JSON.stringify(updatedPendingList));
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="bg-gray-100 border-b-2 border-gray-300">
                    ...
                    <th className="px-4 py-3 font-bold text-gray-700 uppercase text-xs">RMO Name</th>
                ...
                <tbody>
                  {pendingList.map((task, i) => (
                    <tr key={i} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-700">{task.rmoName}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold">{editMode ? 'Edit Task' : 'Assign RMO Task'}</h2>
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  ...
                  {PREDEFINED_RMO_TASKS.map(t => (
                    <option key={t} value={t}>{t}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">RMO Name</label>
                <input
                  type="text"
                  name="rmoName"
